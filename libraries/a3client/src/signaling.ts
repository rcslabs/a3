module a3 {
	...
	export class FlashSignaling implements ISignaling {
		...
		start() {
			...
			swfobject.embedSWF(
				"//webrtc.v2chat.com/a3/RTMP2JS.swf", this._container, "1", "1", MIN_FLASH_VERSION, null,
				this._flashVars || {},
				{
		...
		dtmf(callId, dtmf) {
			if(dtmf.length > 1){
				throw new Error("RTMP signaling unabled to send DTMF string, single char only");
			}
