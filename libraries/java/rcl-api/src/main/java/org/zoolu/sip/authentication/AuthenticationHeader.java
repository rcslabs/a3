...
public abstract class AuthenticationHeader extends Header
{
   ...
   public AuthenticationHeader(String hname, String auth_scheme, List<String> auth_params)
   {  super(hname,auth_scheme);
      if (auth_params.size()>0) value+=" "+auth_params.get(0);
      for (int i=1; i<auth_params.size(); i++) value+=","+LWS_SEPARATOR+auth_params.get(i);
   }
